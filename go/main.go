package main

import (
	"fmt"
)

func main() {
	fmt.Printf("day1 part1: %d\n", day1Part1(readLines(1)))
	fmt.Printf("day1 part2: %d\n", day1Part2(readLines(1)))

	fmt.Printf("day2 part1: %d\n", day2Part1(readLines(2)))
	fmt.Printf("day2 part2: %d\n", day2Part2(readLines(2)))

	fmt.Printf("day3 part1: %d\n", day3Part1(readLines(3)))
	fmt.Printf("day3 part2: %d\n", day3Part2(readLines(3)))

	fmt.Printf("day4 part1: %d\n", day4Part1(readLines(4)))
	fmt.Printf("day4 part2: %d\n", day4Part2(readLines(4)))

	fmt.Printf("day5 part1: %d\n", day5Part1(readLines(5)))
	fmt.Printf("day5 part2: %d\n", day5Part2(readLines(5)))

	fmt.Printf("day6 part1: %d\n", day6Part1(readLines(6)))
	fmt.Printf("day6 part2: %d\n", day6Part2(readLines(6)))

	fmt.Printf("day7 part1: %d\n", day7Part1(readLines(7)))
	fmt.Printf("day7 part2: %d\n", day7Part2(readLines(7)))

	fmt.Printf("day8 part1: %d\n", day8Part1(readLines(8)))
	fmt.Printf("day8 part2: %d\n", day8Part2(readLines(8)))

	fmt.Printf("day9 part1: %d\n", day9Part1(readLines(9)))
	fmt.Printf("day9 part2: %d\n", day9Part2(readLines(9)))

	fmt.Printf("day10 part1: %d\n", day10Part1(readLines(10)))
	fmt.Printf("day10 part2: %d\n", day10Part2(readLines(10)))

	fmt.Printf("day11 part1: %d\n", day11Part1(readLines(11)))
	fmt.Printf("day11 part2: %d\n", day11Part2(readLines(11)))

	fmt.Printf("day12 part1: %d\n", day12Part1(readLines(12)))
	fmt.Printf("day12 part2: %d\n", day12Part2(readLines(12)))

	fmt.Printf("day13 part1: %d\n", day13Part1(readLines(13)))
	fmt.Printf("day13 part2: %d\n", day13Part2(readLines(13)))

	fmt.Printf("day14 part1: %d\n", day14Part1(readLines(14)))
	fmt.Printf("day14 part2: %d\n", day14Part2(readLines(14)))

	fmt.Printf("day15 part1: %d\n", day15Part1(readLines(15)))
	fmt.Printf("day15 part2: %d\n", day15Part2(readLines(15)))

	fmt.Printf("day16 part1: %d\n", day16Part1(readLines(16)))
	fmt.Printf("day16 part2: %d\n", day16Part2(readLines(16)))

	fmt.Printf("day17 part1: %s\n", day17Part1(readLines(17)))
	fmt.Printf("day17 part2: %d\n", day17Part2(readLines(17)))
}
